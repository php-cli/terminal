...
Implement a lightweight dependency injection container for module services. The container supports singleton and
transient bindings, constructor autowiring, and circular dependency detection. This is intentionally simple — not a
full-featured container like Symfony's, but enough for module service management.

**Why this stage**: Modules need to register and resolve services. The container must exist before ModuleRegistry can
wire services.
...
```
src/SDK/Container/
├── ContainerInterface.php     - Contract for DI container
├── Container.php   # Feature Request: Module SDK System
...
The SDK testing leverages the existing `VirtualTerminalDriver` infrastructure for full E2E testing without a real terminal.
...
5. Remove deprecated code
           - Implementation
└── ServiceDefinition.php      - Service binding DTO
```

**MODIFY:**

```
src/SDK/Module/ModuleContext.php - Add container property
```

**CREATE (Tests):**

```
tests/Unit/SDK/Container/
├── ContainerTest.php
└── ServiceDefinitionTest.php
```
...
- PSR-11 `ContainerInterface` - Standard container contract (we'll create our own simpler version)
- `src/UI/Screen/ScreenRegistry.php:17-48` - Registry pattern with has/get methods

### How Container Will Be Used

```php
// In module's boot()
public function boot(ModuleContext $context): void
{
    // Services already registered via ServiceProviderInterface
    $service = $context->container->get(MyService::class);
}

// In module's screens()
public function screens(ContainerInterface $container): iterable
{
    yield $container->get(MyScreen::class);
}
```
...
final class Container implements ContainerInterface
{
    ...
    public function get(string $id): object
    {
        ...
        if (isset($this->resolving[$id])) {
            throw new CircularDependencyException($id, array_keys($this->resolving));
        }
...
Add container property to ModuleContext:

```php
// src/SDK/Module/ModuleContext.php

final readonly class ModuleContext
{
    public function __construct(
        public ContainerInterface $container,  // ADD THIS
        private array $config = [],
    ) {}
...
```php
// src/SDK/Exception/ServiceNotFoundException.php
...
final class ServiceNotFoundException extends \RuntimeException
{
    public function __construct(
        ...
        parent::__construct("Service not found: {$serviceId}");
    }
}
```

### Update CircularDependencyException

```php
// src/SDK/Exception/CircularDependencyException.php

final class CircularDependencyException extends \RuntimeException
{
    /**
     * @param string $serviceId The service that caused the cycle
     * @param array<string> $chain The resolution chain
     */
    public function __construct(
        public readonly string $serviceId,
        public readonly array $chain,
    ) {
        $chainStr = implode(' → ', $chain) . ' → ' . $serviceId;
...
final class ContainerTest extends TestCase
{
    ...
    public function test_bind_returns_new_instance(): void
    {
        $container = new Container();
    ...
    public function test_instance_registers_existing_object(): void
    {
        ...
        $this->assertSame($obj, $container->get('service'));
    }
    ...
    public function test_make_autowires_dependencies(): void
    {
        $container = new Container();
        $container->singleton(DependencyA::class, fn() => new DependencyA());
        
        $service = $container->make(ServiceWithDep::class);
        
        $this->assertInstanceOf(ServiceWithDep::class, $service);
    }
    
    public function test_make_uses_provided_params(): void
    {
        $container = new Container();
    ...
    {
        $container = new Container();
        $container->singleton('dep', fn() => new \stdClass());
        $container->singleton('service', fn($c) => new ServiceWrapper($c->get('dep')));
        
        $service = $container->get('service');
        
        $this->assertInstanceOf(ServiceWrapper::class, $service);
    }
}
```
...
- [ ] `ContainerInterface` with `get()`, `has()`, `make()` methods
- [ ] `Container` implementation with:
    - [ ] `bind()` for transient services
    - [ ] `singleton()` for cached services
    - [ ] `instance()` for pre-existing objects
    - [ ] Circular dependency detection with clear error message
    - [ ] Constructor autowiring in `make()`
- [ ] `ServiceDefinition` DTO with static factory methods
- [ ] `ModuleContext` updated with `container` property
- [ ] `ServiceNotFoundException` updated with `serviceId` property
- [ ] `CircularDependencyException` updated with `chain` property
- [ ] All unit tests pass
- [ ] Static analysis passes

## Dependencies

**Requires**: Stage 1 (ModuleContext, exceptions)

**Enables**:

- Stage 3 (Registry uses Container for service resolution)
- Stage 4 (ServiceProviderInterface uses ServiceDefinition)
- Stage 5 (Builder wires Container)
