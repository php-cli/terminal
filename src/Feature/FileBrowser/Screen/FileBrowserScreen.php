...
final class FileBrowserScreen implements ScreenInterface
{
    ...
    public function render(Renderer $renderer): void
    {
        ...
        // Account for global menu bar (top) and status bar (bottom)
        $panelY = 1;
        $panelHeight = $height - 2; // Subtract menu and status bars
        $leftPanelWidth = (int) ($width * 0.50); // 50/50 split
        $rightPanelWidth = $width - $leftPanelWidth;

        // Render left panel (file list)
        $this->leftPanel->render($renderer, 0, $panelY, $leftPanelWidth, $panelHeight);

        // Render right panel (file preview)
        $this->rightPanel->render($renderer, $leftPanelWidth, $panelY, $rightPanelWidth, $panelHeight);
    }
